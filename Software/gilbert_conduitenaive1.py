...
class Driver():
	...
	def update(self):
		...
		print(struct.pack(struct_fmt,self.current_angle,self.current_speed,))
...
def main2():
	...
	tableau_lidar_mm = [0]*360 #création d'un tableau de 360 zéro
	t0 = time.time()
	drive.set_vitesse_m_s(1.0)
	drive.set_direction_degre(18)
	time.sleep(1)
	drive.set_vitesse_m_s(-1.0)
	drive.set_direction_degre(-18)
	time.sleep(1)
	...
	try : 
		for scan in lidar.iter_scans(scan_type='express') : 
			...
			drive.set_direction_degre(angle_degre)
			vitesse_m_s = 1
			drive.set_vitesse_m_s(vitesse_m_s)
			t1 = time.time()-t0
			print(t1)	
			##############################################
